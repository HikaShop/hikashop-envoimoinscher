...
?><input type="hidden" name="lang_file_override" value="<?php echo @$this->element->shipping_params->lang_file_override;?>" />
	...
	<td id="shipping_services_list">
			...
			<?php
					$opts = array();
					//we display all drop off points for each carrier
	...
	<tr>
		...
		<td>
			<?php
			$options = array("colis"=>"Colis ou Paquet (poids < 70 kg)", "pli"=>"Pli ou courrier (enveloppe : poids < 3 kg)", "encombrant"=>"Encombrant (poids > 70 kg)"/*, "palette"=>"Palette (30 kg < poids palette < 1 000 kg)"*/);
			$opts = array();
			foreach($options as $key => $value){
				$opts[] = @JHTML::_('select.option',$key,$value);
			}
	...
	<tr>
		...
		<td>
			...
			if(empty($this->element->shipping_params->emc_login) || empty($this->element->shipping_params->emc_password) || empty($this->element->shipping_params->api_key)){
				...
				unset($options[100]);
				$opts = array();
				//we disable the main catagories
				foreach($options as $key => $value){
					if($key%1000 != 0)
						$opts[] = @JHTML::_('select.option',$key,$value);
					else
